...
function isMinimum (v, p, schema) {
  ...
  return !(v < p || schema && schema.exclusiveMinimum && v <= p)
}

function isMaximum (v, p, schema) {
  if (typeof v !== 'number') return true

  return !(v > p || schema && schema.exclusiveMaximum && v >= p)
}
...
module.exports = {
  ...
  'number': {
    'minimum':    (p) => (v) => isMinimum(v, p)    ? Success(v) : Failure([{'minimum': null}]),
    'maximum':    (p) => (v) => isMaximum(v, p)    ? Success(v) : Failure([{'maximum': null}]),
