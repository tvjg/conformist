...
class SchemaNode {
  constructor ({env, schemaStack}) {
    ...
    this._validators = typeValidators.concat(formatValidators, requiredValidators, stringValidators)
    this._partialValidators = notPartials
  }
  ...
  combine (childFns) {
    // TODO Compose order will be reversed for required properties
    let childValidators = R.zipWith(R.compose, this._partialValidators, childFns)

    return this.validators.concat(childValidators)
  }
  ...
  get children () {
    ...
    // FIXME Remove cheese
    return this.prop('not').map((s) => [{ env: this.env, schemaStack: [s] }]).getOrElse([])
  }
