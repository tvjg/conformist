...
class SchemaNode {
  ...
  combine (childFns) {
    ...
    anyOfValidators = (anyOfCount > 0) ? [this.env.logical.anyOf(anyOfSlice)] : []
    oneOfValidators = (oneOfCount > 0) ? [this.env.logical.oneOf(oneOfSlice)] : []
    allOfValidators = (allOfCount > 0) ? [this.env.logical.allOf(allOfSlice)] : []
