...
const {map, partial, compose} = require('ramda')
const {mapM} = require('control.monads')

const Endo = f => ({
  f: f,
  concat: (e) => Endo(compose(f, e.f))
})
...
Tree.prototype.foldr = function (f, empty) {
  return this.foldMap(compose(Endo, f), Endo.empty).f(empty)
}
